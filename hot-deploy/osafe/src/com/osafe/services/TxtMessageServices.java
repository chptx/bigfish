...
public class TxtMessageServices {
    ...
    public static Map sendTxtMessage(DispatchContext dctx, Map context) 
    {
        ...
        if (UtilValidate.isEmpty(textMessageMode) || textMessageMode.equals("NONE"))
        {
        	errMsg = UtilProperties.getMessage("OSafeAdminUiLabels", "TxtMsgMethodNoneError", locale);
        ...
        String userId = Util.getProductStoreParm(productStoreId, "TXT_MESSAGE_USER_ID");
        String apiId = Util.getProductStoreParm(productStoreId, "TXT_MESSAGE_API_ID");
        String password = Util.getProductStoreParm(productStoreId, "TXT_MESSAGE_PASSWORD");
        String httpUrl = Util.getProductStoreParm(productStoreId, "TXT_MESSAGE_API_URL");
        String httpMsgUrl = Util.getProductStoreParm(productStoreId, "TXT_MESSAGE_API_MSG_URL");
    ...
    public static Map<String, Object> sendTxtMessageFromTemplate(DispatchContext dctx, Map context) 
    {
        ...
        {   
        	...
        	if(!isPartyTextPreferenceTrue)
        	{
            	return ServiceUtil.returnFailure(UtilProperties.getMessage("OSafeAdminUiLabels", "TxtMsgCustomerTxtPreferenceError", UtilMisc.toList(partyId), locale));
        	}
        	if(isPartyTextPreferenceTrue && UtilValidate.isNotEmpty(toCell))
        	{
	            GenericValue content = delegator.findByPrimaryKeyCache("Content", UtilMisc.toMap("contentId", contentId));
	            if (UtilValidate.isEmpty(content))
	            {
	            	return ServiceUtil.returnFailure(UtilProperties.getMessage("OSafeAdminUiLabels", "TxtMsgTemplateNotFoundError", locale));
	            }
	            if (content.getString("statusId").equals(""))
	            {
	            	return ServiceUtil.returnFailure(UtilProperties.getMessage("OSafeAdminUiLabels", "TxtMsgTemplateStatusError", locale));
	            }
				...
				{
	            	return ServiceUtil.returnFailure(UtilProperties.getMessage("OSafeAdminUiLabels", "TxtMsgTemplateEmptyError", locale));
				}
    ...
    public static String getPartyMobileContactNumber(GenericValue party, Delegator delegator)
    {
    	...
    	try
    	{
            ...
            if (UtilValidate.isNotEmpty(partyPurposeMobilePhones)) 
            {
            	...
            	toCell = telecomNumber.getString("areaCode")+""+telecomNumber.getString("contactNumber");
            	if(UtilValidate.isNotEmpty(countryTeleCode))
            	{
            		toCell = countryTeleCode + "" + toCell;
            	}
            }
    	}
    	catch(Exception e)
