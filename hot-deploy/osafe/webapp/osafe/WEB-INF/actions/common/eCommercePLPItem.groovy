...
if(UtilValidate.isNotEmpty(plpItem)) 
{
  ...
  if (UtilValidate.isNotEmpty(product)) 
  {
	...
	if (UtilValidate.isNotEmpty(inventoryMethod) && "BIGFISH".equals(inventoryMethod.toUpperCase()))
	{
		...
		bfProductAllAttributes = product.getRelated("ProductAttribute");
		if (UtilValidate.isNotEmpty(bfProductAllAttributes))
		{
			bfTotalInventoryProductAttributes = EntityUtil.filterByAnd(bfProductAllAttributes,UtilMisc.toMap('attrName','BF_INVENTORY_TOT'));
			bfWHInventoryProductAttributes = EntityUtil.filterByAnd(bfProductAllAttributes,UtilMisc.toMap('attrName','BF_INVENTORY_WHS'));
			if (UtilValidate.isNotEmpty(bfTotalInventoryProductAttributes))
			{
				bfTotalInventoryProductAttribute = EntityUtil.getFirst(bfTotalInventoryProductAttributes);
				if (UtilValidate.isNotEmpty(bfTotalInventoryProductAttribute))
				{
					bfTotalInventory = bfTotalInventoryProductAttribute.attrValue;
