...
if (UtilValidate.isNotEmpty(rowOrderItem))
{
	...
	shipGroupAssocations=delegator.findByAndCache("OrderItemShipGroupAssoc",UtilMisc.toMap("orderId", rowOrderItem.orderId, "orderItemSeqId", rowOrderItem.orderItemSeqId));
  	shipGroupAssoc = EntityUtil.getFirst(shipGroupAssocations);
  	if (UtilValidate.isNotEmpty(shipGroupAssoc))
  	{
      shipGroup = EntityUtil.getFirst(delegator.findByAndCache("OrderItemShipGroup",UtilMisc.toMap("orderId", rowOrderItem.orderId, "shipGroupSeqId", shipGroupAssoc.shipGroupSeqId)));
	...
	context.idx = request.getAttribute("idx");
	context.rowClass = request.getAttribute("rowClass");
