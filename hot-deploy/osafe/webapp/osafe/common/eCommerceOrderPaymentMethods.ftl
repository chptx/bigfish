	      ...
	      <#else>
	       <div>
	        <input type="hidden" name="paymentMethodId" value="${paymentMethodId}" />
              ...
              <div id="js_remainingPayment" class="remainingPayment">
	        ...
	        <#if (remainingPayment &gt; 0)>
    	        ...
    	        <#if !paymentMethod?has_content>
                  ...
                  <div class ="js_paymentOptions" <#if showPaymentOption == "false">style="display:none"</#if>>
                   <h4>${uiLabelMap.PaymentOptionsHeading}</h4>
                    <div class="entry">
                      <input type="radio" id="js_payInStoreN" name="payInStore" value="N" <#if (!(parameters.payInStore?exists && parameters.payInStore?string == "Y"))>checked="checked"</#if>/><span class="radioOptionText">${uiLabelMap.PayNowLabel}</span>
                      <input type="radio" id="js_payInStoreY" name="payInStore" value="Y" <#if ((parameters.payInStore?exists && parameters.payInStore?string == "Y"))>checked="checked"</#if>/><span class="radioOptionText">${uiLabelMap.PayInStoreLabel}</span>
                    </div>
                  </div>
                  <div id="js_checkoutPaymentOptions" <#if ccAvailable == "false">style="display:none"</#if>>
    	              <!-- EBS section Starts-->
                      ...
                      <#if Static["com.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_PAYNETZ")>
                          <div class="entry ebs">
                            ...
                            <a href="javascript:submitCheckoutForm(document.${formName!}, 'PNZ', 'EXT_PAYNETZ');">
                                <img class="paymentMethodImg" alt="PAYNETZ Checkout" src="http://www.atomtech.in/images/logo.png">
                            </a>
                          </div>
                      </#if>
    	              ...
    	              <#if Static["com.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_PAYPAL")>
    		            <div class="paymentOption paypalEntry">
    		          ...
    		          <#if Static["com.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_CC")>
    		              <div class="paymentOption js_creditCardEntry">
    		          ...
    		          <!-- CREDIT CARD-->
    	              <div class ="paymentOption js_codOptions" <#if (shoppingCart?has_content && shoppingCart.getOrderAttribute("STORE_LOCATION")?has_content) || !(Static["com.osafe.util.Util"].isProductStoreParmTrue(request,"CHECKOUT_ALLOW_COD"))>style="display:none"</#if>>
