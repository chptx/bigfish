    ...
    <div class="entry long">
      ...
      <div class="entryInput checkbox medium">
        ...
        <#list directoryNameList as directoryName>
          <input class="checkBoxEntry" type="checkbox" id="view${directoryName}" name="mediaType"  value="${directoryName}" <#if (parameters.mediaType?exists && context.get('view${directoryName!}')?has_content) || ((intiCb?exists) && (intiCb == "N"))>checked="checked"</#if>/>${directoryName}
        </#list>
      </div>
    </div>
</div>
