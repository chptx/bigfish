...
if (UtilValidate.isNotEmpty(partyId))
{
	...
	if (UtilValidate.isNotEmpty(party))
	{
		...
		if (UtilValidate.isNotEmpty(partyContactMechPurpose))
		{
			...
			partyPostalAddressLocations = EntityUtil.getRelated("PartyContactMech", partyContactMechPurpose);
			partyPostalAddressLocations = EntityUtil.filterByDate(partyPostalAddressLocations,true);
			partyPostalAddressLocations = EntityUtil.orderBy(partyPostalAddressLocations, UtilMisc.toList("fromDate DESC"));
			partyPostalAddressLocations = EntityUtil.getRelatedByAnd("ContactMech", UtilMisc.toMap("contactMechTypeId", "POSTAL_ADDRESS"), partyPostalAddressLocations);
			if (UtilValidate.isNotEmpty(partyPostalAddressLocations))
			{
				partyPostalAddressLocation = EntityUtil.getFirst(partyPostalAddressLocations);
				postalAddressList = partyPostalAddressLocation.getRelated("PostalAddress");
				postalAddress = EntityUtil.getFirst(postalAddressList);
				context.postalAddress = postalAddress;
				context.postalAddressContactMechId = partyPostalAddressLocation.contactMechId;
			}
		}
	}
